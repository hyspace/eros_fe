...
void main() {
  ...
  group('ordinary download flow', () {
    ...
    test('an explicit retry bypasses the previously cached image', () async {
      await cacheImage();
      await download(retry: true);
      expect(processor.fetchCount, 1);
      expect(processor.downloads.single.useCache, isFalse);
    });
    ...
    test('a 403 retry does not re-enable cache for an explicit re-download',
        () async {
      await cacheImage();
      processor.failFirstDownload = true;
      await download(retry: true);
